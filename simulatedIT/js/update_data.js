...
$("#e1,#lam1,#e2,#lam2,#thicknessFactorSlider,#thicknessFactorInput,#alphaSlider,#alphaInput,#lorCheck,#lorentzianSlider,#lorentzianInput,#normCheck,#normEnergyInput1,#normEnergyInput2,#preEdgeInput").on('input', function(e) { 
	...
	wrapperWidth = document.getElementById('wrapperID').offsetWidth;
  	plot = Bokeh.Plotting.figure({
		title: "",
		x_range: new Bokeh.Range1d({ start: plot.x_range.start, end: plot.x_range.end }),
		y_range: new Bokeh.Range1d({ start: plot.y_range.start, end: plot.y_range.end }),
		x_axis_label: "Energy (eV)",
		y_axis_label: "IT (counts)",
		tools: tools,
		height: (2/3) * wrapperWidth,
		width: wrapperWidth,
		margin: ''
...
//////////////////////////////////////////////////////////////////////  CONVOLUTION:
	...
	mu_t_output = [];
	var simulatedITListP = []; /// * my addition
        for (var i = 0; i < energy.length; i++) {
            var iTaccumulator = 0;
            var i0accumulator = 0;
	    var lorAccumulator = 0; /// * my addition
		...
		lorAccumulator += lorValue /// * my addition
            }
            // calculate mu_t:
            mu_t_output.push( Math.log( i0accumulator / iTaccumulator ) );
	    simulatedITListP.push( iTaccumulator ); /// * my addition; also, append """ / lorAccumulator """ after iTaccumulator if you want to normalize
        }

} else {
////////////////////////////////////////////////////   NO CONVOLUTION:
	mu_t_output = [];
	var simulatedITListP = [];
	for (var i = 0; i < energy.length ; i++) {
...
////////////////////////////////////////////////////
	...
	newSource2 = new Bokeh.ColumnDataSource({
	    data: { x: energy, y: simulatedITListP }
	});
